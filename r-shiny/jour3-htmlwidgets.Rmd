...
- Formatage de la date (avec `formatDate()` - notez l'utilisation de la fonction `decimal_date()` du package `lubridate`)
- Formatage d'une colonne (avec `formatStyle()`)
- Ajout d'une barre en fonction de la valeur dans la colonne `median` (toujours avec `formatStyle()`)
...
Pour aller plus loin, il est aussi possible de sélectionner des lignes et d'utiliser cette sélection dans un graphique ou une autre table. Ci-dessous, après avoir fait un résumé de la table, par ville, nous l'affichons, ainsi qu'un graphique. Et lorsque nous naviguons dans la table, les points des villes affichées changent de couleur. De même, lorsque nous sélectionnons des villes, celles-ci voient la taille de leur point grossir.
...
Ajouter, à l'application réalisée sur les données `txhousing` les éléments suivants :
...
- un onglet avec 
    - une table résumant les villes
        ...
        - d'autres informations à réfléchir
